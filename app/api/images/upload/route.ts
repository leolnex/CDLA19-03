...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_FILE_SIZE) {
      return NextResponse.json({ error: 'file_too_large', max: MAX_FILE_SIZE }, { status: 400 })
    }
    ...
    if (!role || !['service_hero', 'project_cover', 'project_gallery'].includes(role)) {
      return NextResponse.json({ error: 'invalid_role' }, { status: 400 })
    }
    
    // Validate slot for service_hero
    let slot: number | null = null
    if (role === 'service_hero') {
      if (!slotStr) {
        return NextResponse.json({ error: 'slot_required_for_hero' }, { status: 400 })
      }
      slot = parseInt(slotStr, 10)
      if (isNaN(slot) || slot < 1 || slot > 4) {
        return NextResponse.json({ error: 'invalid_slot', expected: '1-4' }, { status: 400 })
      }
    }
    ...
    const imageId = await executeQuery(async (pool) => {
      const result = await pool.request()
        .input('owner_type', sql.NVarChar(20), ownerType)
        .input('owner_id', sql.Int, ownerId)
        .input('role', sql.NVarChar(20), role)
        .input('slot', sql.Int, slot)
        .input('mime_type', sql.NVarChar(100), 'image/webp')
        .input('width', sql.Int, specs.width)
        .input('height', sql.Int, specs.height)
        .input('bytes', sql.VarBinary(sql.MAX), processedImage)
        .query(`
          INSERT INTO cdla_images (owner_type, owner_id, role, slot, mime_type, width, height, bytes)
          OUTPUT INSERTED.image_id
          VALUES (@owner_type, @owner_id, @role, @slot, @mime_type, @width, @height, @bytes)
        `)
      
      return result.recordset[0].image_id as number
    })
