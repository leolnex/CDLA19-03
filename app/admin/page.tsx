...
import { FolderOpen, Store, Users, TrendingUp, Phone, Mail, MapPin, MessageSquare } from 'lucide-react'
      ...
      <Card className="border-border">
        ...
        <CardContent>
          {recentLeads.length === 0 ? (
            <p className="py-8 text-center text-foreground/60">{t.admin.noLeadsYet}</p>
          ) : (
                ...
                >
                  <div>
                    ...
                    <div className="mt-1 flex items-start gap-1 text-sm text-foreground/70">
                      ...
                      <span>{categoryLabels[lead.service][language]} - {lead.message.substring(0, 100)}...</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`rounded-full px-2 py-1 text-xs font-medium ${
                      ...
                      {lead.status === 'nuevo' ? 'Nuevo' : lead.status === 'contactado' ? 'Contactado' : 'Cerrado'}
                    </span>
                  </div>
                </div>
              ))}
